...
import { AnalysisRequestSchema, type AnalysisResult } from "../shared/types";
// import { TextEncoder, TextDecoder } from "util";

type Env = {
  DB: any; // Replace 'any' with your actual DB type if available
};
...
app.post('/api/analyze', zValidator('json', AnalysisRequestSchema), async (c) => {
  try {
    ...
    const cached = await c.env.DB.prepare(
      'SELECT * FROM analysis_results WHERE content_hash = ? ORDER BY created_at DESC LIMIT 1'
    ).bind(contentHash).first();

    if (cached) {
      const result: AnalysisResult = {
        trustScore: Number(cached.trust_score),
        status: String(cached.status) as any,
    ...
    await c.env.DB.prepare(
      ...
      JSON.stringify(analysisResult.analysis?.keywords || []),
      JSON.stringify(analysisResult.analysis?.flags || []),
...
function generateMockSources(_content: string, _trustScore: number) {
  const allSources = [
